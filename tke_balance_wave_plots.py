...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 25 11:56:20 2020
...
epsilon = np.array([np.nanmean( (1/(np.sum(intmask,axis = 0)*0.001))*np.trapz(data['epsilon'][:,idx,i]*intmask, 
                                         np.flipud(data['z'][:,idx]), axis = 0)) for i in range(8) ])

k = np.array([np.nanmean(np.trapz((1/(np.sum(intmask,axis = 0)*0.001))*data['tke'][:,idx,i]*intmask,
                                  np.flipud(data['z'][:,idx]), axis = 0)) for i in range(8) ] )
...
tke = np.load('tke_balance_wave.npy', allow_pickle = True).item()

tke['Pws'][np.isnan(tke['Pws'])] = 0
tke['Pw'][np.isnan(tke['Pw'])] = 0
tke['wtrs'][np.isnan(tke['wtrs'])] = 0
tke['wtkt'][np.isnan(tke['wtkt'])] = 0
    
scale = (1/(np.sum(intmask,axis = 0)*0.001))

Pws = scale*np.trapz(tke['Pws'][:,idx]*intmask, np.flipud(data['z'][:,idx]), axis = 0)
...
wtrs = scale*np.trapz(tke['wtrs'][:,idx]*intmask, np.flipud(data['z'][:,idx]), axis = 0)

wtkt = scale*np.trapz(tke['wtkt'][:,idx]*intmask, np.flipud(data['z'][:,idx]), axis = 0)
