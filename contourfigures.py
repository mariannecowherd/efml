...
for N in waveturb.keys():
        for i in range(8):
            try:
                wave_nd_old = waveturb[N][i]['uw1_wave']/ubs[N]**2
                nd_old = waveturb[N][i]['uw1']/ustars[N]**2
                delta = np.sqrt(2*nu_fits[N]/omegas[N])
                delta = np.sqrt(2e-6/omegas[N])
                dudz_old = waveturb[N][i]['dudz']/ubs[N]/delta
                #
                wave_nd_old = wave_nd_old[np.where(~np.isnan(wave_nd_old))]
                nd_old = nd_old[np.where(~np.isnan(wave_nd_old))]
                #
                t_old = waveturb2[N]['uw1']/ustars[N]**2
                tw_old = waveturb2[N]['uw1_wave']/ubs[N]**2
                #
...
z_mesh, y_mesh = np.meshgrid(znew,phasebins3)
levuw1 = np.linspace(np.nanmin(uwmesh),np.nanmax(uwmesh),20);
plt.figure(figsize=(15,10))
cf = plt.contourf(y_mesh, z_mesh, uwmesh.T, levuw1, extend='both')
plt.colorbar(cf)
