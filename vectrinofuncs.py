...
def contour_interp(data,z,norm):
    ...
    data = np.reshape(data,(30,382,8))

    zinterp = np.linspace(0.001,0.015,15)
    data_interp = np.zeros((len(zinterp),P))
    ...
    for i in range(zinterp.size):
        ...
        for j in range(len(idx)):
            u0 = norm[j]
            uz[j,:] = data[idx[j],j,:]/(u0**2)

        data_interp[i,:] = np.nanmean(uz, axis = 0)
...
def iswaves(N,ifplot):
    ...
    #these should be no waves. method: I looked at them
